...
NodeType checkType(const FeatureModel &fd, const Feature *f) {
  ...
  return NodeType::AND;
>>>>>>> 7239ef51eb2d960ee91c7c0eef63ef15d0dbe21f
}

unordered_map<Feature *, bool> sampleRandomly(const FeatureModel &fd,
                                              unordered_map<Feature *, int> &cc,
                                              random_device &rd) {
...
=======
    if (children.size() == 0) {
      if (sample.find(f) == sample.end()) {
        std::cerr << "The leaf node" << f->getName().str()
                  << "was not uncovered in the sample" << std::endl;
      }
      continue;
>>>>>>> 7239ef51eb2d960ee91c7c0eef63ef15d0dbe21f
    }
    ...
    case NodeType::AND: {
      for (Feature *c : children) {
        ...
        if (!c->isOptional()) {
          ...
          auto numCCchild = cc.find(c);
          if (numCCchild != cc.end() && numCCchild->second > 0) {
            std::bernoulli_distribution b(numCCchild->second /
                                          (numCCchild->second + 1));
            sample[c] = b(rd);
          } else {
            sample[c] = false;
    ...
    case NodeType::OR: {
      ...
      while (s.empty()) {
        ...
        for (Feature *c : children) {
          auto numCChild = cc.find(c);
          if (numCChild != cc.end() && numCChild->second > 0) {
            std::bernoulli_distribution b(numCChild->second /
                                          (numCChild->second + 1));
            if (b(rd)) {
              s.push_back(c);
            }
    ...
    case NodeType::XOR: {
      vector<double> weights;
      std::vector<vara::feature::Feature *> childVec(children.begin(),
                                                     children.end());
      for (Feature *c : children) {
        auto numCCchild = cc.find(c);
        weights.push_back(static_cast<double>(numCCchild->second));
