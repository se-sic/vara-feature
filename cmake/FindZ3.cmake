...
function(check_z3_version z3_include z3_lib)
  ...
  file(
    WRITE ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeTmp/testz3.c
    "#include <assert.h>
        #include <z3.h>
        int main() {
  ...
  if(Z3_COMPILED)
    string(REGEX REPLACE "([0-9]*\\.[0-9]*\\.[0-9]*\\.[0-9]*)" "\\1" z3_version
                         "${SRC_OUTPUT}"
    )
...
# If Z3 has not been found in LLVM_Z3_INSTALL_DIR look in the default
# directories
find_path(
  Z3_INCLUDE_DIR
  NAMES z3.h
...
# First, try to check it dynamically, by compiling a small program that prints
# Z3's version
...
if(NOT Z3_VERSION_STRING
  ...
  string(REGEX REPLACE "^.*Z3_BUILD_VERSION[\t ]+([0-9]).*$" "\\1" Z3_BUILD
                       "${z3_version_str}"
  )
...
if(NOT Z3_VERSION_STRING)
  ...
  # conservative and force the found version to 0.0.0 to make version checks
  # always fail.
