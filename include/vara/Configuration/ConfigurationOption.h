...
///
/// Note that this class is not linked to real features from the feature model and does also not check the type and
/// the value of the given option.
class ConfigurationOption {
 public:
  virtual ~ConfigurationOption() = default;

  ConfigurationOption(std::string Name, std::string Value) : Name(std::move(Name)), Value(std::move(Value)) {};
  ...
  /// This method returns the string representation of this configuration option.
  /// \returns the string representation in the format: <code>name: value</code>
