...
static llvm::cl::opt<OutputFormat, false>
    OutputFormatOption(
        "format",
        llvm::cl::desc("The way how the configurations should output."),
        llvm::cl::values(
            clEnumValN(OutputFormat::YAML, "yaml",
  ...
  if (!OutputFilePath.empty() && !Configurations.empty()) {
    ...
    switch (OutputFormatOption.getValue()) {
      case OutputFormat::YAML:
        Str = vara::sampling::SampleSetWriter::writeConfigurations(*FM,
                                                                   Configurations);
        break;
      case OutputFormat::CSV:
        Str = vara::sampling::SampleSetWriterCSV::writeConfigurations(*FM, Configurations);
        break;
    }
