...
public:
  ...
  FeatureTreeItem static *createFeatureTreeItem(vara::feature::FeatureTreeNode* Item);
...
signals:
  ...
  void addCild(vara::feature::Feature *Feature);
protected:
  FeatureTreeItem(vara::feature::FeatureTreeNode* Item ,vara::feature::FeatureTreeNode::NodeKind Kind): Kind(Kind) {
    for(auto *Child : Item->children()){
      if(vara::feature::Relationship::classof(Child)) {
        auto child  = createFeatureTreeItem(dynamic_cast<vara::feature::Relationship*>(Child));
        Children.push_back(child);
        child->setParent(this);
      }else {
        auto child  = createFeatureTreeItem(dynamic_cast<vara::feature::Feature*>(Child));
        Children.push_back(child);
        child->setParent(this);
      }
    }
...
virtual ~FeatureTreeItemFeature() = default;
  FeatureTreeItemFeature(vara::feature::Feature* Item): FeatureTreeItem(Item,vara::feature::FeatureTreeNode::NodeKind::NK_FEATURE), Item(Item) {}
...
FeatureTreeItemRelation(vara::feature::Relationship* Item): FeatureTreeItem(Item,vara::feature::FeatureTreeNode::NodeKind::NK_RELATIONSHIP),Item(Item) {}
[[nodiscard]] QVariant data(int Column) const override{
    if(Column==0) {
